...
{
	avl_pp head = (avl_pp)calloc(1, sizeof(avl_p));//改
	*head = NULL;
	...
	while (root) {
		...
		if (key < root->keyNode) {
			...
			if (!root->left) {
				...
				root->left = static_cast<avl*>(calloc(1, sizeof(avl_t)));//改
				...
				while ((p = (nodedata_p)pop(stack)) != NULL) {//改
			...
			n = (nodedata_p)malloc(sizeof(nodedata));//改
			n->node = root;
			n->direction = LEFT;
			push(stack, n);
			...
			if (!root->right) {
				root->right = static_cast<avl*>(calloc(1, sizeof(avl_t)));//改
...
int search_BFS_avl(avl_pp root, int key, bool stop)
{
	...
	while ((node = (avl_p)dequeue(queue)) != NULL) {//改
		if (!stop)
			log(INFO, "tracking...\n");
...
avl_pp_S init_avl_S(void)
{
	...
	head.avlRoot = static_cast<avl_pp>(calloc(1, sizeof(avl_p)));//改
	*head.avlRoot = NULL;
